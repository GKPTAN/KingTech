...
const DepartmentPage = () => {
    ...
    const filtersToShow = categoryFilters.filter((filterGroup) => {
        ...
        if (Array.isArray(filterGroup.departments)) {
            return filterGroup.departments.some(dep => lowerSubcategory === dep.toLowerCase());
        }
        return false;
    });
  return (
        ...
        <main className="products-content">
            ...
            <div className="catalog">
                <aside className="filters">
                    {filtersToShow.map((filterGroup, idx) => filterGroup.childrens.map((filterCategory) => (
                        <Filter 
                ...
                <div className={`catalog-products ${typeView === 'list' ? 'list' : 'grid'}`}>
                    <p className="quantity-products">{products.length < 1000 ? products.length : `+ de ${Math.floor(products.length / 1000) * 1000}`} produtos encontrados</p>
                    <div className="view-switching">
                        <h3>Ver como:</h3>
                        <div>
                            Lista
                            <span className={`list ${typeView === 'list' ? 'active' : ''}`} onClick={() => setTypeView('list')}>
                                <CiBoxList size={20}/>
                            </span>
                            <span className={`grid ${typeView === 'grid' ? 'active' : ''}`} onClick={() => setTypeView('grid')}>
                                <CiGrid41 size={20}/>
                            </span>
                            Grade
                        </div>
